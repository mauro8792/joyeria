...
@section('content')
    ...
    <div class="container div_trans8 corner4 mt-4 mb-4 p-4">
        <div class="section">
            <h2 class="text-white text-center">Orden de Pedidos</h2>
                                        ...
                                        <div class="row">
                                            <div class="col-sm-10">
                                                ...
                                                @if($cart->status === "Active")
                                                    <a href="{{ url('/carts/'.$cart->cart_id.'/select') }}" class="btn btn-sm btn-outline-danger">{{$cart->status}}</a>
                                                @elseif($cart->status === "Finalizado")
                                                    <a href="{{ url('/carts/'.$cart->cart_id.'/select') }}" class="btn btn-sm btn-outline-primary">{{$cart->status}}</a>
                                                @else
                                                    <a href="{{ url('/carts/'.$cart->cart_id.'/select') }}" class="btn btn-sm btn-outline-warning">{{$cart->status}}</a>
                                                @endif
                                            </div>
                                        </div>
                                    </h2>
